...
if __name__ == '__main__':
    ...
    # added way to move around bad lines
    ...
    # context embedding
    ctx_embs = np.zeros((len(unique_ctx), HIDDEN_DIM_SIZE), dtype=np.float32)
    ...
    for ctx_batch in tqdm(batches(unique_ctx, BATCH_SIZE), desc='Processing ctx',
        ...
        ctx_batch_embs = list(
            elmo.embed_sentences_last_layer(ctx_batch_tok, BATCH_SIZE))

        # Compute average and store in unique ctx vectors (these don't change)
        ctx_batch_embs_avg = [uw_average(emb, nlptok) for emb, nlptok in zip(
            ctx_batch_embs, ctx_batch_nlp)]
        ...
        for ctx, ctx_idx, ctx_emb, offsets, context_tokens in zip(ctx_batch, ctx_batch_idx, ctx_batch_embs,
            ...
            for _, row in vuamc_ctx.iterrows():
                ...
                v_emb = list(elmo.embed_sentences_last_layer(
                    [[row.verb_lemma]], BATCH_SIZE))[0][0]
                if row.subject:
                    s_emb = list(elmo.embed_sentences_last_layer(
                        [[row.subject]], BATCH_SIZE))[0][0]
                else:
                    s_emb = np.zeros_like(ctx_emb[0])
                if row.object:
                    o_emb = list(elmo.embed_sentences_last_layer(
                        [[row.object]], BATCH_SIZE))[0][0]
                else:
